package xyz.skynetcloud.cyberx.commands.util;
...
public class TeleporterDimension extends Teleporter {
		...
		public static final int DIMID = ModConfig.DIMENSION_CYBER_ID;
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag
									? BlockInit.DARK_STEEL_BLOCK.getDefaultState()
	...
	public static class BlockTutorialPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == BlockInit.DARK_STEEL_BLOCK
					|| getBlock(par1World, par2 + 1, par3, par4) == BlockInit.DARK_STEEL_BLOCK) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == BlockInit.DARK_STEEL_BLOCK
					|| getBlock(par1World, par2, par3, par4 + 1) == BlockInit.DARK_STEEL_BLOCK) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
				...
				for (l = -1; l <= 2; ++l) {
					for (i1 = -1; i1 <= 3; ++i1) {
						...
						if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
							...
							if (flag) {
								if (j1 != BlockInit.DARK_STEEL_BLOCK) {
									return false;
								}
							}
							/*
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != BlockInit.DARK_STEEL_BLOCK) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == BlockInit.DARK_STEEL_BLOCK) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != BlockInit.DARK_STEEL_BLOCK || getBlock(par1World, par2 - b0, par3, par4
								- b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != BlockInit.DARK_STEEL_BLOCK || getBlock(par1World, par2 + b0, par3,
										par4 + b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
			...
			if (par5Entity.getRidingEntity() == null && !par5Entity.isBeingRidden() && par5Entity instanceof EntityPlayerMP) {
				...
				if (thePlayer.timeUntilPortal > 0) {
					...
					thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, DIMID,
							new TeleporterDimension(thePlayer.mcServer.getWorld(DIMID)));
				} else {
					thePlayer.timeUntilPortal = 10;
					thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0,
							new TeleporterDimension(thePlayer.mcServer.getWorld(0)));
				}
			}
		}
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		...
		protected Biome[] allowedBiomes = {Biomes.BEACH,Biomes.BIRCH_FOREST,Biomes.BIRCH_FOREST_HILLS};
